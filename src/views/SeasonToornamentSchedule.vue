  ...
  <v-container fluid>
    <v-card>
      ...
      <v-expansion-panels v-else accordion flat>
        <v-expansion-panel
          v-for="stage in stagesWithRounds"
          :key="stage.id"
        >
          <v-expansion-panel-header class="font-weight-bold">
            {{ stage.name }}
            <span class="grey--text ml-2 font-weight-regular">
              ({{ stage.rounds.length }} round{{ stage.rounds.length > 1 ? 's' : '' }})
            </span>
                  ...
                  <template v-else-if="matchesByRound[round.id]">
                    ...
                    <v-card outlined class="mb-3">
                      <v-card-text class="pb-2">
                        <div class="text-caption grey--text mb-2">{{ $t("Toornament.ScheduleSetDate") }}</div>
                        <v-row dense align="center">
                          ...
                          <v-col cols="12" sm="auto">
                            <v-btn
                              ...
                              :disabled="!bulkDate[round.id] || !bulkTime[round.id]"
                              @click="saveRound(round)"
                            >
                              <v-icon left small>mdi-calendar-multiple-check</v-icon>
                    ...
                    <v-simple-table dense>
                      ...
                      <tbody>
                        <tr v-for="match in matchesByRound[round.id]" :key="match.id">
                          <td>{{ match.number }}</td>
...
export default {
  ...
  methods: {
    async onRoundToggle(round) {
      ...
      const all = await this.GetToornamentMatches(this.seasonId, { stage_id: round.stage_id });
      ...
      for (const m of matches) {
        if (m.scheduled_datetime) {
          ...
          this.$set(this.matchDatetime, m.id,
    ...
    async saveRound(round) {
      ...
      const result = await this.ScheduleToornamentRound(this.seasonId, round.id, iso);
      if (result?.message) {
    ...
    async saveMatch(match) {
      ...
      const iso = this.buildIso(date, time);
      const result = await this.ScheduleToornamentMatch(this.seasonId, match.id, iso);
      if (result?.message) {
        this.snackbar = { show: true, text: result.message, color: "success" };
      } else {
        this.snackbar = { show: true, text: this.$t("Toornament.ScheduleError"), color: "error" };
